...
# Fraud classification threshold — raise above default 0.5 to reduce

# over-prediction by Logistic Regression (precision 0.441 at 0.5).

# At 0.6 the model flags only claims it is genuinely confident about.
...
# Apply calibrated threshold instead of default 0.5
...
“”“Build Excel file with 2-3 sheets. Returns BytesIO or None.”””
if not HAS_EXCEL:
return None
buf = io.BytesIO()
# Determine the fraud probability column name defensively
prob_col = ‘Fraud_%’ if ‘Fraud_%’ in results_df.columns else ‘Fraud_Probability_%’
...
```
    fraud_df = results_df[results_df['Prediction']=='FRAUDULENT'].sort_values(
        prob_col, ascending=False)
    # Only write the Fraudulent sheet if there are rows — openpyxl
    # raises "At least one sheet must be visible" when all sheets
    # end up hidden, which can happen with an empty DataFrame.
    if len(fraud_df) > 0:
