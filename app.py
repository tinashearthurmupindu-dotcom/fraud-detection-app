...
if "bg_color" not in st.session_state:
    st.session_state["bg_color"] = "#F0F4FF"
if "bg_label" not in st.session_state:
    st.session_state["bg_label"] = "❄️ Ice Blue"
...
DARK_BG      = st.session_state.get("bg_color", "#F0F4FF")  # user-chosen background
...
ACCENT_TEAL   = "#0D7377"   # original teal kept

# ── Plotly shared theme ───────────────────────────────────────────────
PLOT_LAYOUT = dict(
    paper_bgcolor="rgba(0,0,0,0)",
    plot_bgcolor="rgba(240,244,255,0.7)",
...
.swatch:hover { transform: scale(1.18); border-color: #1D4ED8; }
.swatch.active { border-color: #0D7377 !important; transform: scale(1.1); }
</style>
""", unsafe_allow_html=True)

# Render picker in a container pinned top-right via CSS fixed positioning
with st.container():
    # Build swatch HTML for display (decorative only — real picking via selectbox)
    swatch_html = ""
    for label, color in BG_PRESETS.items():
        if color is None:
            continue
        is_active = "active" if st.session_state["bg_color"] == color else ""
        swatch_html += (
            f'<div class="swatch {is_active}" style="background:{color};" '
            f'title="{label}"></div>'
        )
    st.markdown(f"""
    <div class="color-nav">
        <div class="color-nav-title">🎨 Background Colour</div>
        <div class="color-swatches">{swatch_html}</div>
    </div>
    """, unsafe_allow_html=True)

# Actual interactive picker (slim, right-aligned via columns)
_spacer, _picker_col = st.columns([6, 1])
with _picker_col:
    _chosen_label = st.selectbox(
        "BG",
        list(BG_PRESETS.keys()),
        index=list(BG_PRESETS.keys()).index(st.session_state.get("bg_label","❄️ Ice Blue")),
