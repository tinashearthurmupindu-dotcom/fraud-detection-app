...
DARK_BG      = "#060E1E"   # page background
DARK_SURFACE = "#0D1B2E"   # cards / panels
DARK_BORDER  = "#1E3A5F"   # borders
DARK_PANEL   = "#112244"   # slightly lighter panels

# ── Plotly shared theme ───────────────────────────────────────────────
PLOT_LAYOUT = dict(
    paper_bgcolor="rgba(0,0,0,0)",
    plot_bgcolor="rgba(11,30,61,0.4)",
    font=dict(family="JetBrains Mono, monospace", color="#94a3b8", size=11),
    margin=dict(l=40, r=20, t=40, b=40),
    colorway=["#0D7377","#B91C1C","#C9A84C","#166534","#1a3a6b","#8b5cf6"],
)
...
.metric-card .mc-val {{
    font-family:'DM Serif Display',serif;
    font-size:2rem; color:{TEAL}; line-height:1.1;
    margin-bottom:2px;
}}
.metric-card .mc-lbl {{
...
/* ── Explain cards ── */
.explain-card {{
    background:{DARK_SURFACE}; border:1px solid {DARK_BORDER};
    border-radius:9px; padding:0.9rem 1.1rem; margin:0.4rem 0;
}}
.explain-card .factor-name {{
    font-weight:600; font-size:0.84rem; color:#E2E8F0;
}}
...
/* ── Upload zone ── */
.upload-zone {{
    background:{DARK_SURFACE}; border:2px dashed {DARK_BORDER};
...
.footer {{
    ...
    border-top:1px solid {DARK_BORDER}; margin-top:2.5rem;
}}

/* ── Tab styling ── */
.stTabs [data-baseweb="tab-list"] {{
    gap:3px; background:{DARK_SURFACE};
...
.kpi-value {{
    font-family:'JetBrains Mono',monospace;
    font-size:1.75rem; font-weight:500; color:#E2E8F0; line-height:1;
}}
.kpi-label {{
    font-size:0.7rem; color:{GREY};
    text-transform:uppercase; letter-spacing:0.1em;
...
.dev-card {{
    background:linear-gradient(135deg,#112244,{NAVY});
...
def preprocess_both(df_in, threshold=0.5):
    """Run BOTH models and return results for side-by-side display."""
    lr_preds, lr_probs, scaled, df_proc = preprocess(df_in, lr_model, threshold)
    rf_preds, rf_probs, _, _            = preprocess(df_in, rf_model, threshold)
...
if page == "📊 Dashboard":
    ...
    with ch1:
        ...
        if HAS_PLOTLY:
            ...
            fig.patch.set_facecolor('#060E1E'); ax.set_facecolor('#0D1B2E')
            mets = ['Accuracy','Precision','Recall','F1']
            lrv  = [0.700,0.441,0.837,0.577]
            rfv  = [0.785,0.636,0.286,0.394]
            x    = np.arange(4); w=0.3
    ...
    with ch2:
        ...
        if HAS_PLOTLY:
            ...
            fig2.patch.set_facecolor('#060E1E'); ax2.set_facecolor('#0D1B2E')
    ...
    with p1:
        ...
        if HAS_PLOTLY:
            ...
            fig3.patch.set_facecolor('#060E1E')
    ...
    with p2:
        ...
        if HAS_PLOTLY:
            ...
            fig4.patch.set_facecolor('#060E1E')
    ...
    if analyse_btn:
        ...
        for mc, mpred, mprob, mrisk, mname, mclr in [
            ...
            with mc:
                ...
                st.markdown(f"""
                <div style="background:#0f2744;border:2px solid {mclr};
                  ...
                  <div style="margin-top:0.7rem;background:#1a3a5f;border-radius:6px;
        ...
        st.markdown(f"""
        <div style="background:#0f2744;border:1px solid {agree_clr};border-radius:8px;
        ...
        fig_wf.patch.set_facecolor('#060E1E')
        ax_wf.set_facecolor('#0D1B2E')
        top_wf  = contribs[:8]
        ...
        try:
            ...
            if go2:
                ...
                for mc, mn, nf, nl, nh, avg in [
                    ...
                    with mc:
                        st.markdown(f"""
                        <div style="background:#0f2744;border:2px solid {bc_color};
                          ...
                          <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.6rem">
                            <div style="text-align:center;padding:0.6rem;background:#1a3a5f;border-radius:8px">
...
# ════════════════════════════════════════════════════════════════════
    ...
    with et1:
        ...
        fig7.patch.set_facecolor('#060E1E'); ax7.set_facecolor('#0D1B2E')
    ...
    with et2:
        ...
        if exp_btn:
            ...
            fig_e.patch.set_facecolor('#060E1E'); ax_e.set_facecolor('#0D1B2E')
            top_e  = contribs_e[:10]
...
# ════════════════════════════════════════════════════════════════════
    ...
    st.info("🔗 Full graph analysis requires a live Neo4j connection. "
...
<div class="about-section">
  <div class="about-content">
    ...
    <div style="margin-top:1rem; font-family:'JetBrains Mono',monospace;
      ...
      Tinashe A. Mupindu · Dr S. Jere (Supervisor) · 2026
    </div>
  </div>
</div>""", unsafe_allow_html=True)
