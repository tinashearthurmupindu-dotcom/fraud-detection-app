...
def preprocess(df_in, model):
    ...
    # Drop target and ID columns FIRST before anything else
    drop_always = ['fraud_reported','fraud','is_fraud',
                   'policy_number','policy_bind_date','incident_date',
                   'incident_location','insured_zip','auto_model','_c39']
    for col in drop_always:
        if col in df.columns:
            df.drop(columns=[col], inplace=True)

    # Encode only columns that exist AND are in the encoder dict
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "Both models run on every claim.<br>"
        "Final score = weighted ensemble<br>"
        "(LR 65% + RF 35%).</small>",
        unsafe_allow_html=True)
    ...
    <small style='color:white'>Recall: 83.7% · F1: 57.7%</small>
    """, unsafe_allow_html=True)
    st.markdown("""
    <small style='color:#94A3B8'>
    Random Forest<br>
    </small>
    <small style='color:white'>Accuracy: 78.5% · Prec: 63.6%</small>
    """, unsafe_allow_html=True)

    # Keep chosen_model for backward compat but always run both
...
# ════════════════════════════════════════════════════════════════════
    ...
    if go:
        ...
        preds_lr, probs_lr, _, _ = preprocess(df_in, lr_model)
        preds_rf, probs_rf, _, _ = preprocess(df_in, rf_model)

        lr_prob  = float(probs_lr[0])
        rf_prob  = float(probs_rf[0])
        # Weighted ensemble: LR 65% (better recall), RF 35% (better precision)
        ens_prob = round(0.65 * lr_prob + 0.35 * rf_prob, 4)
        ens_pred = 1 if ens_prob >= 0.5 else 0
        risk     = risk_label(ens_prob)
        models_agree = (lr_prob >= 0.5) == (rf_prob >= 0.5)
        ...
        with m1:
            st.markdown(f"""
            <div class="metric-card">
              <div class="mc-bar" style="background:{TEAL}"></div>
              <div class="mc-val" style="font-size:1.4rem;
                color:{'#B91C1C' if lr_prob>0.5 else '#166534'}">
                {lr_prob*100:.1f}%
              </div>
              <div class="mc-lbl">Logistic Regression</div>
        ...
        with m2:
            st.markdown(f"""
            <div class="metric-card">
              <div class="mc-bar" style="background:{GOLD}"></div>
              <div class="mc-val" style="font-size:1.4rem;
                color:{'#B91C1C' if rf_prob>0.5 else '#166534'}">
                {rf_prob*100:.1f}%
              </div>
              <div class="mc-lbl">Random Forest</div>
              <small style="color:#94A3B8;font-size:0.7rem">
        ...
        with m3:
            ...
            <div class="metric-card">
              <div class="mc-bar"
                style="background:{CRIMSON if ens_pred==1 else SAGE}">
              </div>
              <div class="mc-val" style="font-size:1.4rem;
                color:{CRIMSON if ens_pred==1 else SAGE}">
                {ens_prob*100:.1f}%
              </div>
              <div class="mc-lbl">Ensemble Score</div>
              <small style="color:#94A3B8;font-size:0.7rem">
                LR 65% + RF 35%
              </small>
            </div>""", unsafe_allow_html=True)
        ...
        with m4:
            agree_color = SAGE if models_agree else GOLD
            agree_text  = "✅ Agree" if models_agree else "⚠️ Disagree"
            st.markdown(f"""
            <div class="metric-card">
              <div class="mc-bar" style="background:{agree_color}"></div>
              <div class="mc-val" style="font-size:1.2rem;
                color:{agree_color}">
                {agree_text}
              </div>
              <div class="mc-lbl">Model Consensus</div>
              <small style="color:#94A3B8;font-size:0.7rem">
                {risk} risk level
              </small>
            </div>""", unsafe_allow_html=True)
        ...
        if not models_agree:
            st.markdown(
                ...
                padding:0.9rem 1.2rem;margin:0.8rem 0;font-size:0.85rem;
                color:#92400E">
                <strong>⚠️ Models disagree on this claim.</strong><br>
                Logistic Regression says
                <strong>{'FRAUD' if lr_prob>0.5 else 'LEGITIMATE'}
                ({lr_prob*100:.1f}%)</strong> while
                Random Forest says
                <strong>{'FRAUD' if rf_prob>0.5 else 'LEGITIMATE'}
                ({rf_prob*100:.1f}%)</strong>.
                This happens on borderline claims. The ensemble score
                ({ens_prob*100:.1f}%) is used as the final verdict,
                but <strong>manual review is especially recommended
                when models disagree.</strong>
                </div>""",
                unsafe_allow_html=True)
        ...
        if pred == 1:
            ...
            <div class="verdict-fraud">
              ...
              Ensemble fraud probability: <strong>{prob*100:.1f}%</strong>
              &nbsp;·&nbsp; Risk level: <strong>{risk}</strong><br>
              Recommended action:
              <strong>Refer to Fraud Investigation Unit
        ...
        try:
            ...
            if go2:
                with st.spinner(f"Scoring {n} claims with both models..."):
                    preds_lr2, probs_lr2, _, _ = preprocess(df_up, lr_model)
                    preds_rf2, probs_rf2, _, _ = preprocess(df_up, rf_model)

                # Weighted ensemble
                probs2  = 0.65 * probs_lr2 + 0.35 * probs_rf2
                preds2  = (probs2 >= 0.5).astype(int)
                agree2  = (probs_lr2 >= 0.5) == (probs_rf2 >= 0.5)
                ...
                res.insert(3,'Prediction', ['FRAUDULENT' if p==1 else 'LEGITIMATE'
                                             for p in preds2])
                res.insert(4,'LR_%',       (probs_lr2*100).round(1))
                res.insert(5,'RF_%',       (probs_rf2*100).round(1))
                res.insert(6,'Consensus',  ['✅' if a else '⚠️' for a in agree2])
                ...
                for col,(val,lbl,clr) in zip([s1,s2,s3,s4,s5,s6],[
                    (n,          "Total",       NAVY),
                    (n_fraud,    "Fraud",       CRIMSON),
                    (n_legit,    "Legit",       SAGE),
                    (n_high,     "High Risk",   GOLD),
                    (f"{avg_p:.1f}%","Avg Prob",TEAL),
                    (n_disagree, "⚠️ Review",   "#D97706"),
                ]):
                    with col:
                        st.markdown(f"""
                        <div class="metric-card">
                ...
                for _, row in disp.iterrows():
                    ...
                    display_rows.append({
                        'Verdict':     row['Verdict'],
                        'Ensemble %':  p,
                        'LR %':        float(row.get('LR_%', p)),
                        'RF %':        float(row.get('RF_%', p)),
                        'Consensus':   row.get('Consensus','—'),
                        'Risk Level':  f"{icon} {r}",
                        'Claim Amt':   f"${int(row['total_claim_amount']):,}" if 'total_claim_amount' in row else '—',
                        'Severity':    row.get('incident_severity','—'),
                        'Witnesses':   row.get('witnesses','—'),
                        'Police Rpt':  row.get('police_report_available','—'),
                        'Tenure(mo)':  row.get('months_as_customer','—'),
                    })
                ...
                # Always show CSV — works without any extra libraries
                with e2:
                    st.download_button(
                        "📥 Download CSV",
                    ...
                    st.caption("Always available — open in Excel")

                # Excel — only if openpyxl installed
                with e1:
                    summary_dict = {
                    ...
                    if excel_buf:
                        ...
                        st.info(
                            "💡 Excel export: add `openpyxl` to "
                            "requirements.txt on GitHub and redeploy. "
                            "Use CSV download above in the meantime."
                        )
